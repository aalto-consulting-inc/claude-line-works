...
> **現在の対応範囲**: このプラグインが対応している LINE WORKS の機能は**掲示板(Board)の読み取りのみ**です。
> そのため登録に必要なスコープも `board.read` だけです(トーク・カレンダー等は未対応)。将来機能が増えた場合はスコープの追加が必要になります。
...
2. 「アプリの新規追加」からクライアントアプリを作成(名前の例: `Claude LINE WORKS 連携`)
   - アプリの種類を聞かれたら **「認証アプリ」を選択**してください
     - **認証アプリ** = LINE WORKS API を呼び出すためのアプリ。OAuth でアクセストークンを取得して LINE WORKS API(現状は掲示板 API)にアクセスする、このプラグインの用途はこちら
     - **プロビジョニングアプリ** = ID プロビジョニング(SCIM)連携用。Okta / Entra ID などからのアカウント自動同期のためのもので、**今回は使いません**
3. **OAuth Scopes** で掲示板の読み取りスコープ(`board.read`)を追加
   - 現状のプラグインが使うスコープはこれだけです
...
- プラグインができるのは現状、掲示板の**読み取りのみ**です。投稿・編集・削除はできません
